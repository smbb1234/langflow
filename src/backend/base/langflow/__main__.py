...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    title = "[bold]Welcome to JAI[/bold]\n"
    ...
    access_link = f"[bold]{status_icon} Open JAI {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        fallback_message = (
            "Welcome to JAI\n\n"
            f"[OK] Open JAI -> {protocol}://{access_host}:{port}"
        )
        try:
            ...
            logger.info("Welcome to JAI")
            logger.info(f"Open JAI: {protocol}://{access_host}:{port}")
