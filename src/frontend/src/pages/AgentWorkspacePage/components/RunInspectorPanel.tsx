...
import { DEFAULT_INSPECTOR_WIDTH } from "../constants";
...
export function RunInspectorPanel({
  ...
  const content = useMemo(() => {
    if (activeTab === "guardrails")
      return <GuardrailsSection run={run} theme={theme} />;
    if (activeTab === "evidence")
      return <EvidenceSection run={run} theme={theme} />;
    if (activeTab === "trace")
      return <TraceSummarySection run={run} theme={theme} />;
    if (activeTab === "memory")
      return <MemorySection run={run} theme={theme} />;
  ...
  const tabDots = useMemo(
    () => ({
      guardrails: run.guardrailChecks.some(
  ...
  return (
    <aside
      ...
      style={{
        ...
        width: width ?? DEFAULT_INSPECTOR_WIDTH,
      }}
    >
      <InspectorTabs
        activeTab={activeTab}
        ...
        tabDots={tabDots}
      />
      <div className="h-[calc(100%-39px)] min-h-0 space-y-3 overflow-y-auto p-4 text-sm">
        {content}
      </div>
    </aside>
  );
