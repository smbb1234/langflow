...
export function RunHeader({ run, theme, onBranchRun, onExportRun, onOpenRunMenu }: RunHeaderProps) {
  ...
  return (
    <header className="flex h-[48px] min-w-0 items-center justify-between border-b px-4" style={{ borderColor: theme.panelBorder }}>
      <div className="flex min-w-0 items-center gap-2 overflow-hidden">
        <h1 className="truncate text-[14px] font-semibold" style={{ color: theme.textPrimary }}>
          {run.title}
        </h1>
        ...
        <span
          className="min-w-0 max-w-[40%] truncate rounded-full border px-2 py-0.5 text-[11px]"
