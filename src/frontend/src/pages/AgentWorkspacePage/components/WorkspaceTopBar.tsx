...
const TOP_BAR_STAGE_TO_INTERNAL: Record<
  (typeof STAGE_LABEL_ORDER)[number],
  string[]
> = {
  Plan: ["Plan", "Intake"],
  Retrieve: ["Retrieve"],
...
function getStageStatus(stages: RunStage[], candidates: string[]): StepStatus {
  return (
    stages.find((stage) => candidates.includes(stage.label))?.status ??
    "PENDING"
  );
}
...
export function WorkspaceTopBar({
  ...
  return (
    ...
    >
      <div
        className="hide-scrollbar flex min-w-0 flex-1 items-center gap-2 overflow-x-auto text-[11px]"
        style={{ color: theme.textSecondary }}
        aria-label="Top bar run context"
      >
        <TopBarPill
          theme={theme}
          style={{
        ...
        <TopBarPill
          ...
          className="shrink-0 whitespace-nowrap"
        >
          <IconLock theme={theme} className="mr-1.5" />
          finance_ro
        </TopBarPill>
        <TopBarPill theme={theme} className="shrink-0 whitespace-nowrap">
          tenant: acme
        </TopBarPill>
        ...
        {normalizedStages.map(({ label, status }) => (
          <TopBarPill
            key={label}
            theme={theme}
            className="h-5 shrink-0 whitespace-nowrap rounded px-1.5 text-[10px]"
