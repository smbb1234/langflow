...
The **Shareable Playground** option exposes the **Playground** for a single flow at the `/playground/$FLOW_ID` endpoint.
...
When **Shareable Playground** is enabled and the flow is public, the **Share** menu can also show an **Agent Workspace** link at `/workspace/$FLOW_ID` for the same flow.
